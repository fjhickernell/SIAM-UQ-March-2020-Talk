...
for iter_i = 1:out_param.maxiter
    ...
    disp(['Number of points so far = ' int2str(npoints)])
    
    %% Plot what we have so far
    gail.RemovePlotAxes
    h = plot(xplot, yplot, '-', 'color', MATLABBlue);
    hold on
    h = [h; plot(x(1:npoints), y(1:npoints), '-', 'color', MATLABGreen)];
    h = [h; plot(x(1:npoints), y(1:npoints), '.', 'color', MATLABOrange)];
    axis([-1 1 -1. 0.3])
    legend(h([1 3 2]),{'\(f(x)\)','data', 'APP\((\textsf{X},\textbf{\textit{y}})\)'}, ...
          'location', 'north','box','off', 'orientation','horizontal')
    pbaspect([1 0.5 1])
    print('-depsc',['sampling-funappxg-' int2str(iter_i) '.eps'])
