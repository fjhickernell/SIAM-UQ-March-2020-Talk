...
\uncover<2->{\alert{But} all we need to compute the integral and credible interval is $\widetilde{\vy}$ and 
	\[
	\lambda_\vtheta =  \widetilde{\vC}_{\vtheta,1}, \qquad \text{where } \vC_{\vtheta,1} = \Bigl(  K_{\vtheta}(\vx_i \ominus \vx_1)\Bigr)_{i=1}^n,  \text{ which only depends on } \kappa(\ell/n) \text{ for } \ell = 0, \ldots, n-1
	\]
\uncover<3->{Moreover, $\bigl( \kappa(\ell/n) \bigr)_{\ell = 0}^{n-1}$ can be computed using one FFT.
}}
...
\begin{frame}{Computing $\kappa(x)$ for $x = 0, 1/n \ldots, 1-1/n$}
	...
	\vkappa_r & = \bigl( \kappa(\ell/n) \bigr)_{\ell = 0}^{n-1} = \mW^H \bigl(2 \zeta(r), \zeta(r,1/n) + \zeta(r,(n-1)/n), \ldots, \zeta(r,(n-1)/n) + \zeta(r,1/n)  \bigr)^T
	\end{align*}
\end{frame}
