...
\uncover<2->{\alert{But} all we need to compute the integral and credible interval is $\widetilde{\vy}$ and 
	\[
	\lambda_\vtheta =  \widetilde{\vC}_{\vtheta,1}, \qquad \text{where } \vC_{\vtheta,1} = \Bigl(  K_{\vtheta}(\vx_i \ominus \vx_1)\Bigr)_{i=1}^n,  \text{ which only depends on } \kappa_r(\ell/n) \text{ for } \ell = 0, \ldots, n-1
	\]
...
\input{ProgramsImages/MVNBayesOut.txt}
 	...
 	These algorithms are implemented in GAIL\footfullcite{ChoEtal17b} and soon QMCPy\footfullcite{QMCPy2020a}
...
\uncover<2->{%
	\only<2-8>{Random function fitting kernel with given $r$, $\theta$
\[
\begin{aligned}
f(\vx) & = \widehat{f}_0  \\
