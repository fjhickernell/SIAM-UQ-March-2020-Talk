...
\vspace{-8ex}
   ...
   \vspace{-4ex}
    ...
    c = \int_{[0,1]^{d} \times [0,1]^{d}} C_{\vtheta}(\vt,\vx) \, \dif \vt \dif \vx > 0, \qquad 
\vc = \int_{[0,1]^{d}} C_{\vtheta}(\mX,\vt) \, \dif \vt \in [0,1]^{n} , \qquad
\mC = C_{\vtheta}(\mX,\mX)  \in [0,1]^{n \times n} \\
\int_{[0,1]^d} f(\vx) \, \dif \vx  \, \big \vert \, (\vf = \vy) \sim \cn\Bigl( m[ 1 - \vc^T \mC^{-1}\vone] + \vc \mC^{-1}\vy, s^2(c - \vc^T \mC^{-1}\vc)\Bigr)
    \end{gather*}
...
\begin{frame}{Infering Gaussian Process Parameters for $\GP(m,s^2C_{\vtheta})$\footfullcite{RatHic19a}}
	content...
\end{frame}
